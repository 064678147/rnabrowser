...
DB_ANNO = 'poplar_new_annotations'  # database name for annotations lookup
DB_ORTHO = 'ortholog_db'		# ortholog DB

# lookup table for gene annotation
DB_ANNO_TABLE = 'pgi_annotation'
DB_ANNO_GENEID_COL = 'pgi'

# lookup table for probeset id
DB_LOOKUP_TABLE = 'at_pgi_lookup'
DB_LOOKUP_GENEID_COL = 'pgi'
...
Y_AXIS = {}
Y_AXIS['poplar'] = "GCOS expression signal (TGT=100, Bkg=20)"
...
GENE_ID_DEFAULT1 = 'PtpAffx.200227.1.S1_s_at'
GENE_ID_DEFAULT2 = 'PtpAffx.20694.2.S1_a_at'
...
# you can use key 'default' for each not individually defined
datasourceFooter = {}
datasourceFooter['default'] = '<ul><li>See <a href="http://bar.utoronto.ca/affydb/BAR_instructions.html#efp">distribution of average expression levels</a> \
                               for the probe sets on the Poplar GeneChip in the <a href="data/%s.xml">samples used for the eFP Browser</a>, \
                               to determine if a given gene is a high or low expresser. The small graph that is shown on the eFP output \
                               in Compare and Absolute modes indicates the highest level of expression for the primary gene in red, and the highest level of \
                               expression of the secondary gene in blue</li></ul>'

# regular expression for check of gene id input (here agi and probeset id allowed)
inputRegEx = "^(Ptp(Affx)?\.\d{1,6}\.\d{1,6}\.(A1|A2|S1|S2))_(x_at|s_at|at|a_at)$|^(eugene3)\.\e{6,12}$|^(estExt_)(Genewise|fgenesh)(1|4)\_(v1|kg|pg|pm)(\.|\_v1\.)C_LG_\w{6,10}$|^(grail3.)\d{8,12}$|^(fgenesh)(1|4)\_(kg|pg|pm)\.C\_(scaffold|LG)\_\w{7,12}$|^(gw1)\.\w{1,6}\.\w{1,4}\.1$|^(POPTR)\_[0-9]{4}s[0-9]{5}\.[1-5]{1}$"
...
GENE_ID1_POS = (25, 140)
GENE_ID2_POS = (25, 155)
GENE_PROBESET1_POS = (280, 140)
GENE_PROBESET2_POS = (280, 155)
GENE_ALIAS1_POS = (0, 0)
GENE_ALIAS2_POS = (0, 0)

defaultDataSource = 'Poplar'
dataDir = 'data_poplar'

dbGroupDefault = 'group1'
# list of datasources in same group to find max signal 
groupDatasource = {}
groupDatasource["group1"] = ['Poplar', 'PoplarTreatment']

# mapping of xml files to show datasource name
groupDatasourceName = {}
groupDatasourceName["group1"] = {'Poplar':'Poplar', 'PoplarTreatment':'Poplar-Treatment'}
