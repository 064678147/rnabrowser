...
def checkATH1(gene):
    ...
    gene = re.sub("g", "G", gene)
    gene = re.sub("a", "A", gene)
    file = open('%s/geneATH1.txt' % efpConfig.dataDir)
    if gene+'\n' not in file:
        file.close()
        return None
    else:
...
class Gene:
    def __init__(self, id, database, probesetId, geneAlias):
        self.conn = None
        self.connOrtho = None
        self.annotation = None
        self.alias = geneAlias
        self.geneId = None
    ...
    def retrieveGeneData(self, id, andquery):
        ...
        select_cmd = "SELECT t1.%s, t1.probeset FROM %s t1 WHERE (t1.probeset=%%s or t1.%s=%%s) AND t1.date=(SELECT MAX(t2.date) FROM %s t2) %s" % \
            (efpConfig.DB_LOOKUP_GENEID_COL, efpConfig.DB_LOOKUP_TABLE, efpConfig.DB_LOOKUP_GENEID_COL, efpConfig.DB_LOOKUP_TABLE, andquery)
        if (self.alias == None or self.alias == ''):    
            cursor.execute(select_cmd,(id, id))
        else:
            cursor.execute(select_cmd,(self.geneId, self.geneId))
        row = cursor.fetchall()
        cursor.close()
        self.conn = None
        if len(row) > 0:
    ...
    '''
    # name: retriveGeneId
    # desc: If only a gene alias is entered, a lookup is done to retrieve the gene ID name
    '''
    def retrieveGeneId(self, alias):
        if(self.conn == None):
            self.connect(efpConfig.DB_ANNO)
        cursor = self.conn.cursor()
        select_cmd = "SELECT agi FROM agi_alias WHERE alias = %s"
        cursor.execute(select_cmd, (alias))
        row = cursor.fetchall()
        cursor.close()
        self.conn = None
        if (len(row) > 0):
    ...
    def getAnnotation(self):
        if(efpConfig.DB_ANNO == None or efpConfig.DB_ANNO_TABLE == None or self.getGeneId() is None): # annotations db not defined
            return None
        if(self.annotation == None):
            ...
            cursor = self.conn.cursor()
            MaizeConvert1 = re.match("^GRMZM2G[0-9]{6}$", self.getGeneId())
            MaizeConvert2 = re.match("^AC[0-9]{6}\.[0-9]{1}_FG[0-9]{3}$", self.getGeneId())
            MaizeSp1 = "_T01"
...
class Gene_ATX (Gene):
    def __init__(self, id, probesetId, geneAlias):
        Gene.__init__(self, id, "atTax", probesetId, geneAlias)
